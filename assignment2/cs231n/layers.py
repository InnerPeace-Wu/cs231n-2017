...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # one could save "n_std" in the cache instead of "x_var and eps"
        x_mean = np.mean(x, axis=0)
        x_var = np.var(x, axis=0)
        n_std = np.sqrt(x_var + eps)
        x_norm = (x - x_mean) / n_std
...
def batchnorm_backward(dout, cache):
    ...
    dx_norm = dout * gamma
    # [D, ]
    dx_var = np.sum(dx_norm * (x - x_mean), axis = 0)* -1 / (2 * np.sqrt((x_var + eps) ** 3))
    # [D, ]
    dx_mean = - np.sum(dx_norm,axis=0) / np.sqrt(x_var + eps) + dx_var* -2 * np.mean(x - x_mean,axis = 0)

    dx = dx_norm / np.sqrt(x_var + eps) + dx_mean / N + dx_var * 2 * (x - x_mean) / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta = np.sum(dout, axis = 0)
    dx_norm = dout * gamma
    dx = (N * dx_norm - np.sum(dx_norm,axis=0) - x_norm * np.sum(dx_norm * x_norm,axis=0)) / (N * np.sqrt(x_var + eps))
...
def max_pool_forward_naive(x, pool_param):
    ...
    s = pool_param.get('stride', 1)
    hh = int((H - ph) / s + 1)
    ww = int((W - pw) / s + 1)
    out = np.zeros((N, C, hh, ww))
    for ih in range(hh):
        for iw in range(ww):
            x_field = x[:, :, ih*s:ih*s+ph, iw*s:iw*s+pw]
            out[:, :, ih, iw] = x_field.max(axis=(2, 3))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for ih in range(hh):
        for iw in range(ww):
            ...
            x_field = x[:, :, ih*s:ih*s+ph, iw*s:iw*s+pw]
            # [N, C, ph, pw]
            dx[:, :, ih*s:ih*s+ph, iw*s:iw*s+pw] = dout[:, :, ih, iw][:, :, None, None]
            dx[:, :, ih*s:ih*s+ph, iw*s:iw*s+pw] *= (x_field == x_field.max(axis=(2, 3), keepdims=True))
